...
type PagedCommunityInvolvements =
  components["schemas"]["PagedResponseOfCommunityInvolvementResponse"]

const COLUMNS = ["Involvement", "Nature of Involvement", "Date of Activity", "Attachments"]
const PAGE_SIZE = 10
...
export function CommunityInvolvementTable({
  ...
  const { data, loading, error } = useApiQuery<PagedCommunityInvolvements>(
            ...
            {rows.length === 0 ? (
              ...
              rows.map((row) => (
                <tr key={row.id} className="border-b last:border-0 hover:bg-muted/30">
                  <td className="px-4 py-3">{row.involvement}</td>
                  <td className="px-4 py-3">{row.natureInvolvement}</td>
                  <td className="px-4 py-3">{formatDate(row.dateActivity)}</td>
