...
type PagedProfessionalEngagements =
  components["schemas"]["PagedResponseOfProfessionalEngagementResponse"]

const COLUMNS = ["Engagement Type", "Engagement Name", "Remarks", "Attachments"]
const PAGE_SIZE = 10

export function ProfessionalEngagementTable({
  profileId,
}: {
  ...
  const { data, loading, error } = useApiQuery<PagedProfessionalEngagements>(
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center gap-2 px-4 py-10 text-sm text-muted-foreground">
        ...
        Loading professional engagements…
      </div>
    )
  }
            ...
            {rows.length === 0 ? (
              ...
              rows.map((row) => (
                <tr key={row.id} className="border-b last:border-0 hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3">{row.engagementName}</td>
