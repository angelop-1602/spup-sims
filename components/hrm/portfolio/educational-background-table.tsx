...
type PagedEducationalBackgrounds =
  components["schemas"]["PagedResponseOfEducationalBackgroundResponse"]

const COLUMNS = ["Degree Level", "Degree", "Institution", "Date Graduated", "Attachments"]
const PAGE_SIZE = 10
...
export function EducationalBackgroundTable({
  ...
  const { data, loading, error } = useApiQuery<PagedEducationalBackgrounds>(
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center gap-2 px-4 py-10 text-sm text-muted-foreground">
        ...
        Loading educational background…
      </div>
    )
  }
            ...
            {rows.length === 0 ? (
              ...
              rows.map((row) => (
                <tr key={row.id} className="border-b last:border-0 hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3">{row.degree}</td>
                  <td className="px-4 py-3">{row.institution}</td>
                  <td className="px-4 py-3">{formatDate(row.dateGraduated)}</td>
                  <td className="px-4 py-3">
                    <EducationCredentialsCell
                      profileId={profileId}
                      educationalBackgroundId={row.id}
                    />
                  </td>
                </tr>
              ))
